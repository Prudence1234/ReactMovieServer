...
export default class MoviesDAO {
  ...
  /**
   * Gets a movie by countries
   * @param {string[]} countries - The desired movie id, the _id in Mongo
   * @returns {MflixMovie[] | null} Returns movies or nothing
   */
   static async getMoviesByCountry(countries) {
    return await movies.find({
      countries: { $in: countries },
    },
    { projection: { title: 1, _id: 1 } }).toArray();
    } catch (e) {
      console.error(`Something went wrong in getMovieByCountry: ${e}`)
      throw e
    }
}
